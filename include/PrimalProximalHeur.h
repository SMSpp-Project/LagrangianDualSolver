...
/*--------------------------------------------------------------------------*/
 ...
 enum int_par_type_PPH {
  ...
  intRecoveryThreads ,
  ///< threads solving the components in the consensus recovery
  /**< Number of threads that solve the components of f_Block when the
   * primal recovery fixes the copies of a decision to their consensus [see
   * consensus_recovery()], the components being independent then. The
   * default is 1. */
 ...
 *  @{ */
 ...
 PrimalProximalHeur( void ) : LagrangianDualSolver() {
  ...
  RecThreads = get_dflt_int_par( intRecoveryThreads );
...
/*--------------------------------------------------------------------------*/
  ...
  *
  * - intRecoveryThreads [1]: threads solving the components in the
  *                 consensus recovery.
  *
...
/*--------------------------------------------------------------------------*/
 ...
 [[nodiscard]] int get_dflt_int_par( idx_type par ) const override {
  switch( par ) {
   ...
   case( intRecoveryThreads ):  return( 1 );
...
/*--------------------------------------------------------------------------*/
 ...
 [[nodiscard]] int get_int_par( idx_type par ) const override {
  switch( par ) {
   ...
   case( intRecoveryThreads ):  return( RecThreads );
   }
...
/*--------------------------------------------------------------------------*/
 ...
 [[nodiscard]] idx_type int_par_str2idx( const std::string & name )
  ...
  if( name == "intRecoveryThreads" )
   return( intRecoveryThreads );
...
/*--------------------------------------------------------------------------*/
 ...
 [[nodiscard]] const std::string & int_par_idx2str( idx_type idx )
  ...
  static const std::string _rt = "intRecoveryThreads";
  if( idx == intInnerMaxIter )
   return( _mi );
  if( idx == intUseWarmStartPSol )
   return( _ws );
  if( idx == intRecoveryThreads )
   return( _rt );
...
/*--------------------------------------------------------------------------*/
 ...
 int RecThreads;  ///< threads of the consensus recovery
...
/*--------------------------------------------------------------------------*/
 ...
 /// recovers a feasible solution by the consensus of the copies of a decision
 /** The relaxed Constraint of the form x_a - x_b = 0 tie copies of one
  * decision that live in different components, as the non-anticipativity
  * Constraint of a two-stage problem do. Their Variable are grouped by those
  * Constraint, each group is fixed to its mean, rounded if the Variable are
  * integer, and the components, independent then, are solved one by one
  * with the Solver of RecoveryBSC, by RecThreads threads; the sum of their
  * values is the value of a feasible solution, returned in \p cost, and
  * the solution is left in the Variable. Returns false, having changed
  * nothing, if there are no such Constraint or no RecoveryBSC, and false
  * if a component is not solved. */

 bool consensus_recovery( double & cost );
